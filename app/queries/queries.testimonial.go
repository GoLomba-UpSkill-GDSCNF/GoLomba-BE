...
func GetAllTestimony() ([]models.TestimonialResponse, error) {
	...
	if err := database.DB.Table("testimonials").Select("testimonials.id, testimonials.user_id, testimonials.message, testimonials.stars, users.full_name").Joins("left join users ON users.id = testimonials.user_id").Scan(&testimonials).Error; err != nil {
		return nil, err
	}
